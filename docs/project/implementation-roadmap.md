...
| P9.3 | Implement Actual-write application receipts and crash recovery in migration 008 or later; replace only the guaranteed-empty, insert-blocked 006 compatibility parent under the gated protocol | Implementation/security | P2.4, P2.6, P9.2             | Agent after recovery review        | Assert the reserved parent is empty, rebuild strict primary/unique keys and child foreign keys with foreign keys enabled, then pass `foreign_key_check`; general targets, one anchored intent per logical receipt, stable principal/action, unknown quarantine/holds, authoritative recovery/replay |
